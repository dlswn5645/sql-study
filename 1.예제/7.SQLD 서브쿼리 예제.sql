...
-- # 서브쿼리 
-- ## 서브쿼리를 사용할 수 있는 곳 
-- [select절(스칼라 서브쿼리), from절(인라인 뷰), where절, having절, order by 절
-- insert의 value절, update set절]

-- ## 서브쿼리 사용 유의점 
-- 1. 반드시 괄호로 감쌀 것
-- 2. 비교 연산자 오른쪽에 위치시킬 것
...
-- 20200525에 받은 급여가 20200525 기준 전체 평균 이상의 급여를 받고 있는 직원들의 정보  
...
-- 한국데이터베이스진흥원에서 발급한 자격증을 가지고 있는 사원번호와 자격증 개수를 출력 
SELECT
    emp_no, count(*) CNT
FROM tb_emp_certi
WHERE certi_cd IN(
...
WHERE (A.dept_cd, A.birth_de) IN (
                    SELECT E.dept_cd, MIN(E.birth_de) AS MIN_BIRTH
                    FROM tb_emp E
                    GROUP BY E.dept_cd
                    HAVING COUNT(*) >1
                )
...
-- 주소가 강남인 직원들의 부서 정보를 조회 
-- NOT EXISTS: 존재하지 않는 정보를 조회
SELECT
    A.dept_cd, A.dept_nm
FROM tb_dept A
WHERE EXISTS (
                SELECT 1 FROM tb_emp K WHERE K.dept_cd = A.dept_cd
                                        AND K.addr LIKE '%강남%');
SELECT 1, emp_nm, addr FROM tb_emp K WHERE addr LIKE '%강남%';  
